  ...
  async getReyComixPages(comicUrl) {
    try {
      ...
      const comicData = await page.evaluate(() => {
        const title = (document.querySelector("h1") && document.querySelector("h1").textContent) || "Comic"
        const images = []
...
class EnhancedXXXSearch {
    ...
    try {
      ...
      const videoInfo = await page.evaluate(() => {
        const title =
          ...
          (document.querySelector('[class*="title"]') && document.querySelector('[class*="title"]').textContent) ||
          document.title

        const duration =
          (document.querySelector(".duration") && document.querySelector(".duration").textContent) ||
        ...
        const views =
          (document.querySelector(".views") && document.querySelector(".views").textContent) ||
          (document.querySelector('[class*="views"]') && document.querySelector('[class*="views"]').textContent) ||
  ...
  async getEnhancedThumbnail(item) {
    const possibleThumbnails = [
      item.pagemap && item.pagemap.cse_thumbnail && item.pagemap.cse_thumbnail[0] && item.pagemap.cse_thumbnail[0].src,
      item.pagemap && item.pagemap.cse_image && item.pagemap.cse_image[0] && item.pagemap.cse_image[0].src,
      item.pagemap && item.pagemap.metatags && item.pagemap.metatags[0] && item.pagemap.metatags[0]["og:image"],
      item.pagemap && item.pagemap.metatags && item.pagemap.metatags[0] && item.pagemap.metatags[0]["twitter:image"],
    ].filter(Boolean)
  ...
  async createEnhancedAdultEmbed(item, index, total) {
    const title = (item.videoInfo && item.videoInfo.title) || item.title
    const link = item.link
    const context = item.displayLink
    const thumbnail = (item.videoInfo && item.videoInfo.thumbnail) || item.enhancedThumbnail
  ...
  async handleVideoWatch(interaction, cache) {
    ...
    if (currentItem.directVideoUrl || (currentItem.videoInfo && currentItem.videoInfo.directVideoSrc)) {
...
async function getGameIcon(universeId) {
  try {
    ...
    return (
      (response.data.data && response.data.data[0] && response.data.data[0].imageUrl) ||
...
async function handleRobloxServersView(interaction, cache, page = 0) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({
      text: `Página ${page + 1}/${totalPages} | Total: ${totalServers} servidores | API: ${apiInfo ? apiInfo.remaining : 0}/${apiInfo ? apiInfo.max : 0}`,
    })
    .setTimestamp()
...
async function createComicViewerEmbed(comicData, imageIndex) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor("#9b59b6")
    .setFooter({
      text: `Página ${imageIndex + 1}/${pages.length} | API: ${apiInfo ? apiInfo.remaining : 0}/${apiInfo ? apiInfo.max : 0}`,
    })
    .setTimestamp()
...
function addAPIUsageToEmbed(embed, apiType = "google") {
  ...
  if (apiInfo) {
    ...
    embed.setFooter({
      text: `${(embed.data.footer && embed.data.footer.text) || ""} | ${statusEmoji} API: ${apiInfo.remaining}/${apiInfo.max} (${100 - percentage}% disponible)`,
    })
  }
  return embed
}
...
async function handleRobloxSearch(message, args) {
  ...
  try {
    ...
    gameData = gameInfoResponse.data.data && gameInfoResponse.data.data[0]
...
**📈 Información General:**
⭐ Rating: ${(gameData.totalUpVotes && gameData.totalUpVotes.toLocaleString()) || 0}👍 / ${(gameData.totalDownVotes && gameData.totalDownVotes.toLocaleString()) || 0}👎
🎯 Visitas: ${(gameData.visits && gameData.visits.toLocaleString()) || "N/A"}
🎮 Jugando ahora: ${(gameData.playing && gameData.playing.toLocaleString()) || totalPlayers.toLocaleString()}`)
      .setColor("#00b2ff")
      .setThumbnail(gameIcon)
      .setFooter({
...
**📈 Información General:**
⭐ Rating: ${(cache.gameData.totalUpVotes && cache.gameData.totalUpVotes.toLocaleString()) || 0}👍 / ${(cache.gameData.totalDownVotes && cache.gameData.totalDownVotes.toLocaleString()) || 0}👎
🎯 Visitas: ${(cache.gameData.visits && cache.gameData.visits.toLocaleString()) || "N/A"}`)
        .setColor("#00b2ff")
        .setThumbnail(cache.gameIcon)
        .setFooter({
